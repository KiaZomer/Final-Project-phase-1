...
void User::addSalary(int i){
    if (thisFilmIsStrong(i) && filmsPublished[i]->thereIsVoter())
        money += 0.95 * filmsPublished[i]->showPrice();
    if (thisFilmIsNormal(i)&& filmsPublished[i]->thereIsVoter())
        money += 0.9 * filmsPublished[i]->showPrice();
    if (thisFilmIsWeak(i)&& filmsPublished[i]->thereIsVoter())
...
string User::showNameFilmWhithFilmId(int filmID){
    for (int i=0;i<filmsPublished.size();i++)
        if (filmsPublished[i]->ShowFilmId() == filmID)
...
int User::showRateWithFilmId(int filmID){
    for (int i=0;i<filmsPublished.size();i++)
        if (filmsPublished[i]->ShowFilmId() == filmID)
...
void User::addRateToMovie(int filmID, float rate){
    for (int i=0;i<filmsPublished.size();i++)
        if (filmsPublished[i]->ShowFilmId() == filmID)
            filmsPublished[i]->addRate(rate);
    for (int i=0;i<filmsBought.size();i++)
        if (filmsBought[i]->ShowFilmId() == filmID)
            filmsBought[i]->addRate(rate);
}
