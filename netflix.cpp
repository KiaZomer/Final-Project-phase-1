...
const string TRUEE = "true";
const string FALSEE = "false";
...
const string TITLE_PUBLISHED_FILMS="#. Film Id | Film Name | Film Length | Film price | Rate | Production Year | Film Director ";
const string TITLE_FOLLOWERS="List of Followers\n#. User Id | User Username | User Email";
const string NOTIFICATIONS = "notifications";
const string TITLE_NOTFS = "#. Notification Message";
...
const string MIN_RATE = "min_rate";
...
void NetflixSystem::pushBackUser(string name, string email, string pass, int age, int publisher, int insystem){ 
    users.push_back(new User(name,email,pass,age,publisher,1,users.size() + 1));
}
...
string NetflixSystem::showMovieNameOfThisFilm(int filmID){
    for (int i=0;i<users.size();i++)
        for (int j=0;j<users[i]->numOfFilms();j++)
            if (users[i]->showFilmId(j) == filmID)
                return users[i]->showNameFilmWhithFilmId(filmID);
}
...
bool NetflixSystem::isFourCommands(string line){
    return firstString(line) == DELETE
    || firstString(line) == PUT 
    || firstString(line) == GET 
...
void NetflixSystem::filterFilmsPublished(string line){
    if (users.size() == 0) throw PermissionDenied();
...
void NetflixSystem::EditFilm(string line){
    ...
    if (users[whoIsInSystem()]->thisFilmIdExists(stoi(whatYouWant(line,FILM_ID)))){
        ...
        if (stoi(whatYouWant(line,FILM_ID)) <= idfilm) throw PermissionDenied();
        else throw NotFound();
    }
}
...
void NetflixSystem::PostFilm(string line){
    ...
    if (thisStrExists(line,NAME) && thisStrExists(line,LENGTH) && thisStrExists(line,YEAR) && thisStrExists(line,PRICE) &&
        ...
        users[whoIsInSystem()]->addFilms(users[whoIsInSystem()]->showPublisher()
            ...
            stoi(whatYouWant(line,LENGTH)),stoi(whatYouWant(line,PRICE)),idfilm + 1,users[whoIsInSystem()]->showUserID());
...
void NetflixSystem::deleteFilmid(string line){
    ...
    if (!isPublisher(whoIsInSystem())) throw PermissionDenied();
    if (stoi(whatYouWant(line,FILM_ID)) > idfilm) throw NotFound();
...
void NetflixSystem::BuyFilm(string line){
    if (users.size() == 0) throw PermissionDenied();
    if (howManySpace(line)!=4) throw BadRequest();
    if (!thisStrExists(line,FILM_ID)) throw bad_typeid();
    if (stoi(whatYouWant(line,FILM_ID)) > idfilm || thisFilmIDGotDeleted(stoi(whatYouWant(line,FILM_ID)))) throw NotFound();
...
void NetflixSystem::commentOnFilm(string line){
    ...
    if (howManySpace(line) != 6) throw BadRequest();
    if (!thisStrExists(line,FILM_ID)) throw BadRequest();
    if (stoi(whatYouWant(line,FILM_ID)) > idfilm || thisFilmIDGotDeleted(stoi(whatYouWant(line,FILM_ID))) ||
    ...
    string str = "User " + users[whoIsInSystem()]->showUserName() + 
        ...
        showNameFilmWhithFilmId(stoi(whatYouWant(line,FILM_ID)))) + " with id " + whatYouWant(line,FILM_ID) + ".\n";
...
void NetflixSystem::DeleteComment(string line){
    ...
    if (stoi(whatYouWant(line,FILM_ID)) > idfilm || stoi(whatYouWant(line,FILM_ID)) < 0) throw NotFound();
...
void NetflixSystem::rateOnFilm(string line){
    ...
    if (howManySpace(line) != 6 ) throw BadRequest();
    if (!thisStrExists(line,FILM_ID)) throw BadRequest();
    if (thisFilmIDGotDeleted(stoi(whatYouWant(line,FILM_ID))) || stoi(whatYouWant(line,FILM_ID)) > idfilm || 
        stoi(whatYouWant(line,FILM_ID)) <=0) throw NotFound();
    if (!forRateOrCmThisFilmHasBougth(whoIsInSystem(),stoi(whatYouWant(line,FILM_ID)))) throw PermissionDenied(); 
    ...
    users[whoIsInSystem()]->addRateToMovie(stoi(whatYouWant(line,FILM_ID)),stof(whatYouWant(line,SCORE)));
    users[showWhichUserWithFilmId(stoi(whatYouWant(line,FILM_ID)))]->PushBackNotf("User " +
     users[whoIsInSystem()]->showUserName() + " with id " +
        to_string(users[whoIsInSystem()]->showUserID()) + " rate your film " +
         users[showWhichUserWithFilmId(stoi(whatYouWant(line,FILM_ID)))]->
        showNameFilmWhithFilmId(stoi(whatYouWant(line,FILM_ID))) + " with id " +
...
bool NetflixSystem::thisFilmHasBoughtByThisPerson(int filmID, int who){
    for (int i=0;i<users[who]->numOfBoughtFilms();i++)
        if (users[who]->showFilmId(i) == filmID)
            return true;
}
...
void NetflixSystem::showDetailsOfFilm(string line){
    ...
    cout << "Details of Film " <<  users[showWhichUserWithFilmId(stoi(whatYouWant(line,FILM_ID)))]
        ->showNameFilmWhithFilmId(stoi(whatYouWant(line,FILM_ID))) << endl << "Id = " <<
        ...
        numOfMsgsOfThisFilmId(stoi(whatYouWant(line,FILM_ID)),i) > 1)
            ...
            ->numOfMsgsOfThisFilmId(stoi(whatYouWant(line,FILM_ID)),i);j++){
                ...
                ->showCmId(i,stoi(whatYouWant(line,FILM_ID))) << "." << j << ". " <<
                    users[showWhichUserWithFilmId(stoi(whatYouWant(line,FILM_ID)))]->showCm(i,j,stoi(whatYouWant(line,FILM_ID))) << endl;
            }
    }
    ...
    try{
        if (line.length() != 0){
            ...
            if (thisStrExists(line,QUESTION)){
                ...
                if (firstString(line) == PUT && whatYouWant(line,PUT) == FILMS && whatYouWant(line,FILMS) == QUESTION){
                    EditFilm(line);
                    tst = 1;
                }
                if (firstString(line) == DELETE && whatYouWant(line,DELETE) == FILMS && whatYouWant(line,FILMS) == QUESTION){
                    deleteFilmid(line);   
                    tst = 1;
                }
                ...
                if (firstString(line) == DELETE && whatYouWant(line,DELETE) == COMMENT && whatYouWant(line,COMMENT) == QUESTION){
                    DeleteComment(line);
                    tst = 1;
                }
